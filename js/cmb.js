...
(function ($) {
    ...
    CMBMeta = {
         ...
         */
        TimePicker: function () {
            $('.savp_sailen_timepicker').each(function () {
         ...
         */
        DatePicker: function () {
            $('.savp_sailen_datepicker').each(function () {
                $('#' + jQuery(this).attr('id')).datepicker();
				//$('#_savp_sailen_event_start_date').datepicker();
            ...
            $("#ui-datepicker-div").wrap('<div class="savp_sailen_element" />');
        }, // End DatePicker
        /**
         * Initialize color picker
         */
        ColorPicker: function () {
            $('input:text.savp_sailen_colorpicker').each(function (i) {
        ...
        FileUpload: function () {
            var formfield;
            $('.savp_sailen_upload_file').change(function () {
                formfield = $(this).attr('name');
                $('#' + formfield + '_id').val("");
            });
            $('.savp_sailen_upload_button').live('click', function () {
                var buttonLabel;
                formfield = $(this).prev('input').attr('name');
                buttonLabel = 'Use as ' + $('label[for=' + formfield + ']').text();
                tb_show('', 'media-upload.php?post_id=' + $('#post_ID').val() + '&type=file&savp_sailen_force_send=true&savp_sailen_send_label=' + buttonLabel + '&TB_iframe=true');
                return false;
            });
            $('.savp_sailen_remove_file_button').live('click', function () {
            ...
            window.savp_sailen_original_send_to_editor = window.send_to_editor;
            window.send_to_editor = function (html) {
                ...
                if (formfield) {
                    ...
                    if (itemurl.match(image)) {
                        uploadStatus = '<div class="img_status"><img src="' + itemurl + '" alt="" /><a href="#" class="savp_sailen_remove_file_button" rel="' + formfield + '">Remove Image</a></div>';
                    } else {
                        // No output preview if it's not an image
                        // Standard generic output if it's not an image.
                        html = '<a href="' + itemurl + '" target="_blank" rel="external">View File</a>';
                        uploadStatus = '<div class="no_image"><span class="file_link">' + html + '</span>&nbsp;&nbsp;&nbsp;<a href="#" class="savp_sailen_remove_file_button" rel="' + formfield + '">Remove</a></div>';
                    }
                    $('#' + formfield).val(itemurl);
                    $('#' + formfield + '_id').val(itemid);
                    $('#' + formfield).siblings('.savp_sailen_upload_status').slideDown().html(uploadStatus);
                    tb_remove();
                } else {
                    window.savp_sailen_original_send_to_editor(html);
                }
                formfield = '';
            };
        } // End FileUpload
